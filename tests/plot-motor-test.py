...
time1, temp_gear1, temp_motor1, current1, rpm1 = loadfile('egrip-%s-motorsections/section1.csv'%(testid))
time2, temp_gear2, temp_motor2, current2, rpm2 = loadfile('egrip-%s-motorsections/section2.csv'%(testid))
time3, temp_gear3, temp_motor3, current3, rpm3 = loadfile('egrip-%s-motorsections/section3.csv'%(testid))
...
ax1.set_xlabel('time (min.)'); ax1.set_ylabel('Temp. gear (deg. C)'); leg1 = ax1.legend(); ax1.grid(); ax1.set_xlim(xlims)
...
ax2.set_xlabel('time (min.)'); ax2.set_ylabel('Temp. motor (deg. C)');  leg2 = ax2.legend(); ax2.grid(); ax2.set_xlim(xlims)
...
ax3.set_xlabel('time (min.)'); ax3.set_ylabel('Current (A)');  leg3 = ax3.legend(); ax3.grid(); ax3.set_xlim(xlims)
...
ax4.set_xlabel('time (min.)'); ax4.set_ylabel('Speed (RPM)');  leg4 = ax4.legend(); ax4.grid(); ax4.set_xlim(xlims)

ax1.set_title(r'Bench test of EGRIP motor sections (no load) at 100%% throttle, %s'%(testid))

fig.tight_layout()
plt.savefig('motor-test-timeseries-%s.png'%(testid),dpi=200)
...
ax1.set_title('Bench test of EGRIP motor sections (no load) at 100%% throttle, %s'%(testid))

fig.tight_layout()
plt.savefig('motor-test-%s.png'%(testid),dpi=200)
